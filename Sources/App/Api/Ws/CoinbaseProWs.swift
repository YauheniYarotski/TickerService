...
class CoinbaseProWs: BaseWs {
  ...
  func startListenTickers(pairs: [CoinbasePair]) {
    let product_ids = pairs.map({$0.symbol})
    let request = CoinbaseProRequest(type: MessageType.subscribe.rawValue, channels:  [Channel.ticker.rawValue], product_ids: product_ids)
...
fileprivate enum Channel: String {
  case heartbeat, ticker, level2, user, matches, full
}

fileprivate enum MessageType: String, CaseIterable {
  case error = "error"
